...
int CBckGround::Update()
{
	...
	if (m_tInfo.vPos.x + CScrollMgr::Get_Instance()->Get_ScrollX() * 0.3f< m_tInfo.vSize.x * -0.5f)
		m_bDead = true;
...
void CBckGround::Render(HDC _DC)
{
	...
	D3DXMatrixTranslation(&matTrans, m_tInfo.vPos.x + CScrollMgr::Get_Instance()->Get_ScrollX()* 0.3f, m_tInfo.vPos.y, 0.f);
