...
void CCollisionMgr::Collision_Player_Monster(list<CObj*> _Dst, list<CObj*> _Src)
{
	for (auto& diter : _Dst) {
		for (auto& siter : _Src) {
			if (static_cast<CPlayer*>(diter)->Get_Immortal())
				return;
			if (!static_cast<CMonster*>(siter)->IsHit()) {
				if (Collision_OBB(diter, siter)) {
...
void CCollisionMgr::Collision_Player_Obstacle(list<CObj*> _Dst, list<CObj*> _Src)
{
	for (auto& diter : _Dst) {
		for (auto& siter : _Src) {
			if (static_cast<CPlayer*>(diter)->Get_Immortal())
